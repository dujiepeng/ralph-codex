...
- `ralph/ralph.sh` and `ralph/CODEX.md` for running the Codex-only Ralph loop from a target project
...
- `ralph/ralph.sh` -> `ralph/ralph.sh`
- `ralph/CODEX.md` -> `ralph/CODEX.md`
...
- Business logic belongs in `skills/`, `ralph/CODEX.md`, or `ralph/ralph.sh`, not in shell wrappers.
